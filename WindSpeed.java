public class WindSpeed{
    ...
    public double[] windSpeed(double xStartingPressure, double yStartingPressure, int latitude){
        ...
        if(latitude!=0) {
            windVelocity[0] = (xWindVelocityCF / (-2 * angularVelocity * Math.sin(latitude*Math.PI/180)))/conversionFactor;
        }else{
            windVelocity[0]=xWindVelocityCF/conversionFactor;
        }
        ...
        if(latitude!=0) {
            windVelocity[1] = (yWindVelocityCF / (-2 * angularVelocity * Math.sin(latitude*Math.PI/180)))/conversionFactor;
        }else{
            windVelocity[1]=yWindVelocityCF/conversionFactor;
        }
        return windVelocity;
    }
